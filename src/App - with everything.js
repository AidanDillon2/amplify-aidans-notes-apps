...
this.deleteNoteListener = API.graphql(graphqlOperation(onDeleteTodo)).subscribe ({ next:  noteData => {
const deletedNote = noteData.value.data.onDeleteTodo
const updatedNotes = this.state.notes.filter(note => note.id !== deletedNote.id)
this.setState({notes: updatedNotes})

}});
this.updateNoteListener = API.graphql(graphqlOperation(onUpdateTodo)).subscribe ({ next: noteData => { const {notes} = this.state
